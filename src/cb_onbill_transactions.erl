...
maybe_credit_billing_id(Context) ->
    ...
    case wh_services:find_reseller_id(cb_context:account_id(Context)) of
        MasterAccountId ->
            lager:debug("invoking a bookkeeper to add credit"),
            maybe_create_credit_tansaction(Context);
        AuthAccountId when AuthAccountId =/= MasterAccountId ->
            lager:debug("allowing reseller to add credit without invoking a bookkeeper"),
            maybe_create_credit_tansaction(Context);
        ResellerId ->
...
-spec maybe_create_credit_tansaction(cb_context:context()) -> cb_context:context().
maybe_create_credit_tansaction(Context) ->
    case create_credit_tansaction(Context) of
        {'error', _R}=Error ->
...
-spec create_credit_tansaction(cb_context:context()) ->
                                     {'ok', wh_transaction:transaction()} |
                                     {'error', _}.
create_credit_tansaction(Context) ->
    ...
    Reason = wh_json:get_value(<<"reason">>, JObj, <<"manual_addition">>),
    Description = wh_json:get_value(<<"description">>, JObj),

    Routines = [fun(Tr) -> wh_transaction:set_reason(Reason, Tr) end
                ,fun(Tr) -> wh_transaction:set_description(Description, Tr) end
                ,fun(Tr) -> wh_transaction:set_metadata(Meta, Tr) end
                ,fun wh_transaction:save/1
               ],
    lists:foldl(
      fun(F, Tr) -> F(Tr) end
      ,wh_transaction:credit(AccountId, Units)
      ,Routines
     ).
...
maybe_debit_billing_id(Context) ->
    ...
    case wh_services:find_reseller_id(cb_context:account_id(Context)) of
        ...
        AuthAccountId when AuthAccountId =/= MasterAccountId ->
            ...
            maybe_create_debit_tansaction(Context);
        ResellerId ->
            lager:debug("sub-accounts of non-master resellers must contact the reseller to add credit"),
            Resp = wh_json:from_list(
                     [{<<"message">>, <<"Please contact your phone provider to add credit.">>}
                      ,{<<"cause">>, ResellerId}
                     ]),
...
lager:info("IAM cb_tratsactions fetch undefined"),
    case wh_transactions:fetch(cb_context:account_id(Context), From, To) of
        {'error', _R}=Error -> send_resp(Error, Context);
        {'ok', Transactions} ->
lager:info("IAM cb_tratsactions fetch undefined Transactions: ~p",[Transactions]),
            JObjs = wh_transactions:to_public_json(Transactions),
lager:info("IAM cb_tratsactions fetch undefined JObjs: ~p",[JObjs]),
            send_resp({'ok', JObjs}, Context)
    end;
fetch_transactions(Context, From, To, <<"only_calls">>) ->
lager:info("IAM cb_tratsactions fetch only_calls"),
...
fetch_transactions(Context, From, To, Reason) when Reason =:= <<"only_bookkeeper">>; Reason =:= <<"no_calls">> ->
lager:info("IAM cb_tratsactions fetch bookkeeper and calls  only_calls"),
...
lager:info("IAM cb_tratsactions fetch Reason: ~p",[Reason]),
    case wh_transactions:fetch(cb_context:account_id(Context), From, To) of
        {'error', _R}=Error -> send_resp(Error, Context);
        {'ok', Transactions} ->
            Filtered = wh_transactions:filter_by_reason(Reason, Transactions),
