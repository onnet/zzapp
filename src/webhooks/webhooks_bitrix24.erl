...
-define(HOOK_NAME, <<"bitrix24">>).
-define(NAME, <<"bitrix24">>).
-define(DESC, <<"This webhook notifies Bitrix24 about call events">>).
-define(METADATA
...
handle_event(<<"CHANNEL_ANSWER">>, JObj, Hook) ->
    ...
    {'ok', Cookie, _AccountId, _UserId} = bitrix24_auth(Hook),
...
bitrix24_auth(#webhook{uri = URL
    ...
    of
        ...
        {_, Code, _Headers, Body} ->
            lager:info("IAM bitrix24_auth Failed Code: ~p Body: ~p",[Code, kz_json:decode(Body)]),
            {'error', 'auth_failed'}
    end.
...
call_add(JObj, #webhook{custom_data = CustomData}) ->
    ...
    Url = <<"https://2megarts.bitrix24.ru/api/v2/incoming_leads/sip"
           ,"?login=", Code/binary
